...
const char * const BITCOIN_CONF_FILENAME = "spedo.conf";
const char * const BITCOIN_PID_FILENAME = "spedod.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "spedo" is a composite category enabling all Spedo-related debug output
            if(ptrCategory->count(string("spedo"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SpedoCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SpedoCore
    // Mac: ~/Library/Application Support/SpedoCore
    // Unix: ~/.spedocore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SpedoCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SpedoCore";
#else
    // Unix
    return pathRet / ".spedocore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty spedo.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override spedo.conf
